...
unsigned char TSD305_ADDR = 0x1e; // 0b0011110

// TSD305 device commands
unsigned char TSD305_CONVERT_ADCS_COMMAND = 0xaf;

unsigned char TSD305_STATUS_BUSY_MASK = 0x20;
unsigned char TSD305_STATUS_MEMOTY_ERROR_MASK = 0x04;

#define TSD305_CONVERSION_TIME 80//100
...
static const int32_t object_temperature_lut[LUT_AMBIENT_SIZE + 1][LUT_ADC_SIZE + 1] = {
...
bool tsd305::is_connected(void) {
  ...
  //return (Wire.endTransmission() == 0);
	...
	if (flag == 0)
	{
		printf("TDS305 sensor found: 0x%X\n", TSD305_ADDR);
		conn_flag = true;
	}
	else
	{
...
int32_t tsd305::get_lut_at(uint8_t x, uint8_t y) 
{
  //return (int32_t)pgm_read_dword(&(object_temperature_lut[x][y]));
	return (int32_t)(object_temperature_lut[x][y]);
}
